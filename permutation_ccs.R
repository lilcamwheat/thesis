#' Prognosis model - Permutation test
#'
...
permutation.cv.km.ccs <- function(data_test, test=character(0),   k=10){
...
for (l in 1:500) {
  ...
  df2=CEsubset[sample(1:nrow(CEsubset)),]
  new=CEsubset
  new$survtime=df2$survtime
  new$PTD_cat=df2$PTD_cat
  
  ff=1
  for (ff in 1:k) {
    
    met_train <- subset(new, .foldID != ff)
    met_test  <- subset(new, .foldID == ff)
  ...
  svydifftest=svylogrank(Surv(TIME, EVENT) ~ highlabel,design=dpbc)
  
  svystat=as.numeric(svydifftest[[2]][1])
  test=c(test,svystat)
...
# test statistic from log-rank test: 6.14890290
p=length(which(as.numeric(test) >= 6.14890290))/500
print(p)
}
